#!/usr/bin/env bash
# =============================================================================
# deploy.sh — Azure NXT Maxscale — Automatic Deployer v2.6.0
...
show_banner() {
  ...
  for line in \
    ...
    "   ██╔══██║ ███╔╝  ██║   ██║██╔══██╗██╔══╝" \
    "   ██║  ██║███████╗╚██████╔╝██║  ██║███████╗" \
    "        NXT Maxscale — Automatic Deployer v2.6.0"; do
    printf "  ${C_BCYAN}║${C_RESET}"
    _rpad "$line" "$inner"
    printf "${C_BCYAN}║${C_RESET}\n"
  done
...
gen_fpm_conf() {
  local dest="${1:-$INSTALL_DIR/custom-fpm.conf}"
  step "Generating PHP-FPM pool configuration"

  # ── Measure actual PHP-FPM PSS memory (ps_mem.py approach) ──────────────
  # Read /proc/$pid/smaps PSS values for running php-fpm processes.
  # PSS (Proportional Set Size) is the most accurate per-process memory metric:
  # private pages + shared pages divided by the number of processes sharing them.
  local php_pss_kb=0 php_proc_count=0
  for pid in /proc/[0-9]*/cmdline; do
    pid_num="${pid%/cmdline}"; pid_num="${pid_num##*/}"
    [[ -r "$pid" ]] || continue
    cmd=$(tr '\0' ' ' < "$pid" 2>/dev/null | head -c 100)
    [[ "$cmd" == *php-fpm* ]] || continue
    local smaps="/proc/${pid_num}/smaps_rollup"
    [[ -r "$smaps" ]] || smaps="/proc/${pid_num}/smaps"
    [[ -r "$smaps" ]] || continue
    local pss
    pss=$(awk '/^Pss:/{sum+=$2} END{print sum+0}' "$smaps" 2>/dev/null)
    if (( pss > 0 )); then
      php_pss_kb=$(( php_pss_kb + pss ))
      (( php_proc_count++ ))
    fi
  done

  # Per-process PSS in MB (use measured value, fallback to 80 MB default)
  local mem_per_worker_mb=80
  if (( php_proc_count > 0 )); then
    mem_per_worker_mb=$(( (php_pss_kb / php_proc_count + 512) / 1024 ))
    mem_per_worker_mb=$(( mem_per_worker_mb < 40 ? 80 : mem_per_worker_mb ))
    info "PHP-FPM measured PSS: ${mem_per_worker_mb} MB/process (${php_proc_count} processes sampled)"
  else
    info "PHP-FPM: no running processes found — using default ${mem_per_worker_mb} MB/process estimate"
  fi

  # ── Available RAM budget for PHP ─────────────────────────────────────────
  local total_ram_mb
  total_ram_mb=$(awk '/^MemTotal:/{print int($2/1024)}' /proc/meminfo)

  # Reserve ~40% for OS + DB/Redis/RustFS/Signaling, give 60% to PHP nodes
  local php_budget_mb=$(( total_ram_mb * 60 / 100 ))
  local per_node_budget_mb=$(( php_budget_mb / NC_NODES ))

  # ── Calculate pool values ─────────────────────────────────────────────────
  local max_children=$(( per_node_budget_mb / mem_per_worker_mb ))
  # Clamp: minimum 5 (sane floor), maximum 50 (avoid thrashing)
  (( max_children < 5  )) && max_children=5
  (( max_children > 50 )) && max_children=50

  local start_servers=$(( max_children / 4 ));  (( start_servers < 2 )) && start_servers=2
  local min_spare=$(( max_children / 6 ));      (( min_spare    < 1 )) && min_spare=1
  local max_spare=$(( max_children / 2 ));      (( max_spare    < start_servers )) && max_spare=$start_servers

  info "PHP-FPM pool: max_children=${max_children} start=${start_servers} min_spare=${min_spare} max_spare=${max_spare}"
  info "  RAM budget: ${total_ram_mb} MB total → ${per_node_budget_mb} MB/node ÷ ${mem_per_worker_mb} MB/worker × ${NC_NODES} nodes"

  cat > "$dest" <<FPMCONF
; Auto-generated by deploy.sh — do not edit manually
; Calculated from: ${total_ram_mb} MB RAM, ${NC_NODES} FPM nodes,
;   ${mem_per_worker_mb} MB/worker (PSS-measured or estimated)
[www]
pm = dynamic
...
FPMCONF
  ...
  info "custom-fpm.conf generated (pm.max_children=${max_children})"
}
